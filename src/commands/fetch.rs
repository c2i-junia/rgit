...
pub fn fetch(remote_repo_path: &str, branch: &str) {
    ...
    let remote_branch_ref = format!("{}/refs/{}", remote_repo_path, branch);

    // read the remote branch reference to get the latest commit hash
    let remote_commit_hash = fs::read_to_string(&remote_branch_ref)
    ...
    let objects = collect_objects(&repo_path, &remote_commit_hash);

    // check which objects are missing on the remote
    let missing_objects = get_missing_objects(&RepoPath::Local, &objects);

    for object_hash in missing_objects {
        let subdir = &object_hash[0..2];
        let filename = &object_hash[2..];
        let remote_object_path = format!("{}/objects/{}/{}", remote_repo_path, subdir, filename);
        let local_object_dir = format!(".rgit/objects/{}", subdir);
        let local_object_path = format!("{}/{}", local_object_dir, filename);
